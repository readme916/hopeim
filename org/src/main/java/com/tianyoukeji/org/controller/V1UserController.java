...
public class V1UserController extends DefaultHandler {
	...
	public HttpPostReturnUuid enable(@PathVariable(required = true) Long uuid,
			@RequestBody(required = true) ReasonBody body) {
	...
	public HttpPostReturnUuid disable(@PathVariable(required = true) Long uuid, @RequestBody(required = true) ReasonBody body) {
